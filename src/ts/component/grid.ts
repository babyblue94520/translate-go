...
export namespace Grid {
    ...
    export enum ClassName {
        ...
        BarLeft = 'ttb-grid-bar-left',
        BarCenter = 'ttb-grid-bar-center',
        BarRight = 'ttb-grid-bar-right',
        FirstPage = 'ttb-grid-first-page ttb-icon-first',
        PrevPage = 'ttb-grid-prev-page ttb-icon-prev',
        NextPage = 'ttb-grid-next-page ttb-icon-next',
        LastPage = 'ttb-grid-last-page ttb-icon-last',
        PageInfo = 'ttb-grid-page-info',
        RowInfo = 'ttb-grid-row-info',
        PageInput = 'ttb-grid-page-input',
        Disable = 'ttb-grid-disable',
        HasContent = 'ttb-grid-has-content',
        Show = 'ttb-grid-show',
        Selected = 'ttb-grid-selected',
        Empty = 'ttb-grid-empty'
    }
            ...
            if (CommonUtil.isArray(render)) {
                ...
                for (let i in render) {
                    ...
                    if (el != undefined) {
                        if (el instanceof Element) {
                            ...
                            (<HTMLElement>this).appendChild(document.createTextNode(el));
                        }
                    }
                }
            } else {
                if (render != undefined) {
                    if (render instanceof Element) {
                        ...
                        (<HTMLElement>this).appendChild(document.createTextNode(render));
                    }
                }
            }
        } catch (e) {
    ...
    export class GridBuilder {
         ...
         * @param record
         */
        public static buildGridRecordElement<T>(columns: IGridColumn<T>[], index: number, record: T): HTMLTableRowElement {
            ...
            for (let i in columns) {
                ...
                if (config.checkbox) {
                    ...
                    td.appendChild(div);
                    div.style.whiteSpace = config.nowrap ? 'nowrap' : 'normal';
                    if (config.maxWidth) {
    ...
    export class Grid<T> {
        ...
        protected _footerCenterElement: HTMLElement;
        ...
        protected _colSpan = 0;

        protected _checkboxs: IGridChecbox[] = [];
        protected _selectedRecords: T[] = [];
        protected _selectedRecord: T;
        protected _selectedTr: HTMLTableRowElement;

        constructor(config: IConfig<T>) {
            this._config = deepClone(this._config, config);
            // checkbox
            if (this._config.checkbox) {
        ...
        public updateOnLoad(fn: IOnLoad) {
            this._config.onLoad = fn;
        }
         ...
         */
        public initPageable() {
            this._sorts = deepClone(this._defaultSorts);
            this._pageable = {
                size: this._config.size,
                page: 0,
        ...
        protected onCheckedAll(e) {
            ...
            if (this._checkedAllElement.checked) {
                for (let i in this._checkboxs) {
                    ...
                    gridCheckbox.tr.classList.add(ClassName.Selected);
                ...
                for (let i in this._checkboxs) {
                    gridCheckbox = this._checkboxs[i];
                    gridCheckbox.tr.classList.remove(ClassName.Selected);
        ...
        protected initHeaderColumn() {
            ...
            for (let i in this._config.rowColumns) {
                ...
                if (!column) {
                    this._headerElements.push(null);
                    continue;
                }
        ...
        protected initFooter() {
            this._footerCenterElement = document.createElement('div');
            this._footerCenterElement.className = ClassName.BarCenter;
            ...
            this._footerCenterElement.appendChild(this._loadMoreElement);
            this._footerRightElement = document.createElement('div');
            this._footerRightElement.className = ClassName.BarRight;
            ...
            this._footerRightElement.appendChild(document.createTextNode('显示 '));
            ...
            this._footerRightElement.appendChild(document.createTextNode(' 笔，总共 '));
            this._footerRightElement.appendChild(this._rowTotalTextNode);
            this._footerRightElement.appendChild(document.createTextNode('笔'));

            this._footerElement.appendChild(this._footerCenterElement);
        ...
        protected loadTbody(records: T[], tbodyAppendChild, clean: boolean) {
            ...
            if (length > 0) {
                ...
                let record;
                this._colSpan = this._headerElements.length;
                for (let i = startRow; i < endRow; i++) {
                    record = records[i];
                    if (!record) {
                        continue;
                    }
                    tbodyAppendChild(tbody, i, record);
                }
                ...
                tbody.appendChild(GridBuilder.buildEmptyGridRecordElement(this._colSpan));
            }
            if (clean) {
                this._tbodyElement.innerHTML = '';
            }
         ...
         * @param record
         */
        protected tbodyAppendChild = (tbody: Node, i, record): HTMLTableRowElement => {
            ...
            if (!this._config.checkbox) {
                tr.addEventListener('click', (e: MouseEvent) => {
                    ...
                    if (this._selectedTr) {
                        this._selectedTr.classList.remove(ClassName.Selected);
                    }
                    this._selectedTr = tr;
                    this._selectedTr.classList.add(ClassName.Selected);
                });
            }
            if (tbody instanceof DocumentFragment) {
         ...
         * @param colSpan
         */
        protected tbodyAppendChildContent = (tbody, i, record): HTMLTableRowElement => {
            let tr = GridBuilder.buildGridRecordElement(this._columns, i, record);
            tr.addEventListener('click', (e) => {
                ...
                let content = tr.nextElementSibling as HTMLElement;
                let td = tr.nextElementSibling.firstChild as HTMLElement;
                if (td.innerText == '') {
                    td.appendChild(GridBuilder.loadContent(this._columnContents, i, record));
                }
                if (tr.classList.contains(ClassName.Show)) {
                    content.classList.remove(ClassName.Show);
                    tr.classList.remove(ClassName.Show);
                } else {
                    content.classList.add(ClassName.Show);
                    tr.classList.add(ClassName.Show);
                }
            });
            ...
            if (tbody instanceof DocumentFragment) {
                tbody.appendChild(tr);
                tbody.appendChild(GridBuilder.buildGridRecordContentElement(this._colSpan, this._columnContents, i, record));
            } else {
                tbody.replaceChild(tr, tbody.childNodes[i]);
                tbody.replaceChild(GridBuilder.buildGridRecordContentElement(this._colSpan, this._columnContents, i, record), tbody.childNodes[i + 1]);
            }
            return tr;
        }
        ...
        protected checkboxRender = (tr: HTMLTableRowElement, td: HTMLTableCellElement, checkbox: HTMLInputElement, record: T, index: number) => {
            ...
            td.addEventListener('click', (e: Event) => {
                ...
                if (checkbox.checked) {
                    ...
                    tr.classList.add(ClassName.Selected);
                } else {
                    if (i != -1) {
                        this._selectedRecords.splice(i, 1);
                    }
                    tr.classList.remove(ClassName.Selected);
                }
        ...
        public refresh(index: number, record: T) {
            if (this._page && this._page.content && this._page.content.length > index) {
                let old = this._page.content[index];
                if (old) {
                    this._page.content[index] = record;
                    let tr;
                    if (this._columnContents && this._columnContents.length > 0) {
                        tr = this.tbodyAppendChildContent(this._tbodyElement, index, record);
                    } else {
                        tr = this.tbodyAppendChild(this._tbodyElement, index, record);
                    }
                    if (this._selectedRecord == old) {
                        this._selectedRecord = record;
                        this._selectedTr = tr;
                        this._selectedTr.classList.add(ClassName.Selected);
                    }
    ...
    export class PageGrid<T> extends Grid<T> {
        ...
        protected initFooter() {
            ...
            this._footerLeftElement.className = ClassName.BarLeft;
            ...
            DomUtil.addListenOnEnter(this._pageInputElement, this.onPageEnter.bind(this));
            ...
            this._pageInfoElement.appendChild(document.createTextNode('第 '));
            this._pageInfoElement.appendChild(this._pageInputElement);
            this._pageInfoElement.appendChild(document.createTextNode(' 页，共 '));
            this._pageInfoElement.appendChild(this._pageTotalTextNode);
            this._pageInfoElement.appendChild(document.createTextNode(' 页'));
            ...
            this._footerCenterElement.appendChild(this._firstPageElement);
            this._footerCenterElement.appendChild(this._prevPageElement);
            this._footerCenterElement.appendChild(this._pageInfoElement);
            this._footerCenterElement.appendChild(this._nextPageElement);
            this._footerCenterElement.appendChild(this._lastPageElement);
            ...
            this._footerRightElement.appendChild(document.createTextNode('当前 '));
            this._footerRightElement.appendChild(this._rowStartTextNode);
            this._footerRightElement.appendChild(document.createTextNode(' - '));
            this._footerRightElement.appendChild(this._rowEndTextNode);
            this._footerRightElement.appendChild(document.createTextNode(' 笔，总共 '));
            ...
            if (this._config.sizes && this._config.sizes.length > 0) {
                this._footerRightElement.appendChild(document.createTextNode('，每页 '));
                let sizeSelectElement = document.createElement('select');
                sizeSelectElement.addEventListener('change', () => {
                    this.onSizeChange(sizeSelectElement.value);
                });
                if (this._config.sizes.indexOf(this._config.size) == -1) {
                    this._config.sizes.push(this._config.size);
                    this._config.sizes.sort((a, b) => a > b ? 1 : a == b ? 0 : -1);
                }
                let options = '';
                let size;
                for (let i in this._config.sizes) {
                    size = this._config.sizes[i];
                    options += `<option value="${size}" ${this._config.size == size ? 'selected' : ''}>${size}</option>`;
                }
                sizeSelectElement.innerHTML = options;
                this._footerRightElement.appendChild(sizeSelectElement);
            }
